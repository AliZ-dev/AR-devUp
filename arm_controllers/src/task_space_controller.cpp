...
{
class Task_Space_Controller : public controller_interface::Controller<hardware_interface::EffortJointInterface>
{
  public:
    bool init(hardware_interface::EffortJointInterface *hw, ros::NodeHandle &n)
    {
        ...
        for (size_t i = 0; i < n_joints_; i++)
        {
            ...
            if (n.getParam("/elfin/task_space_controller/gains/elfin_joint" + si + "/pid/p", Kp[i]))
            {
                Kp_(i) = Kp[i];
            }
            else
            {
                std::cout << "/elfin/task_space_controller/gains/elfin_joint" + si + "/pid/p" << std::endl;
                ROS_ERROR("Cannot find pid/p gain");
                return false;
            }

            if (n.getParam("/elfin/task_space_controller/gains/elfin_joint" + si + "/pid/i", Ki[i]))
            {
                Ki_(i) = Ki[i];
            }
            else
            ...
            if (n.getParam("/elfin/task_space_controller/gains/elfin_joint" + si + "/pid/d", Kd[i]))
            {
                Kd_(i) = Kd[i];
            }
            else
        ...
        if (ctr_obj_ == 1)
        {
            if (!n.getParam("/elfin/task_space_controller/clik_gain/K_regulation", K_regulation_))
            {
        ...
        {
            if (!n.getParam("/elfin/task_space_controller/clik_gain/K_tracking", K_tracking_))
            {
        ...
        Jdot_.resize(kdl_chain_.getNrOfJoints());
        ...
        // 6.2 subsriber
        sub_x_cmd_ = n.subscribe<std_msgs::Float64MultiArray>("command", 1, &Task_Space_Controller::commandCB, this);
    ...
    void update(const ros::Time &time, const ros::Duration &period)
    {
        ...
        // 2 ways of solving Jacobian derivative. Jdot_ is simply the derivative,
        // and qdot_vel_array is Jacobian_derivative(q_, qdot_) * qdot_.
        ...
        // Convert Jacobian derivative to JntArray to simplify calculations.
        ...
        aux_d_.data = M_.data * J_.data.inverse() * (xd_ddot_ + Kp_.data.cwiseProduct(ex_) + Kd_.data.cwiseProduct(ex_dot_) - J_dot_q_dot_array_.data);
...
PLUGINLIB_EXPORT_CLASS(arm_controllers::Task_Space_Controller, controller_interface::ControllerBase)
